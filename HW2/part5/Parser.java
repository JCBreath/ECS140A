...
public class Parser {
    ...
    private void statement_list() {
    	while( is(TK.TILDE) || is(TK.ID) || is(TK.PRINT) || is(TK.DO) || is(TK.IF) || is(TK.INC) || is(TK.DEC)) {
    	    statement();
    	}
    }
    	...
    	if(is(TK.TILDE)) {
    		...
    		if(is(TK.NUM)) {
              ...
              if(lvl == -1) {
                     ...
                     cur_id = "x_" + i + tok.string;
                     sysprint(cur_id);
    	       mustbe(TK.ID);
              } else { 
                     // global
                     if(lvl == -2) {
                           cur_id = "x_1" + tok.string;
                           sysprint(cur_id);
                     } else { // specific scoping level
                            st.check_scoping(tok.string, tok.lineNumber ,lvl);
                            cur_id = "x_" + (st.stack.size()-lvl) + tok.string;
                            sysprint(cur_id);
                     }
                     mustbe(TK.ID);
              }
